...
static int owcpa_check_ciphertext(const unsigned char *ciphertext)
{
  ...
  t &= 0xff << (8-(7 & (tDiTRU_LOGQ*tDiTRU_PACK_DEG)));//creates the mask to preserve only meaningful bits of last byte setting unused bits to zero 
...
static int owcpa_check_m(const poly *m)
{
  ...
  uint16_t ps = 0;
  uint16_t ms = 0;
  for(i=0; i<N; i++)  //N should be checked again for dihedral
  {
    ps += m->coeffs[i] & 1;
    ms += m->coeffs[i] & 2;
  }
                   ...
                   const unsigned char seed[tDiTRU_SAMPLE_FG_BYTES])
{
  ...
  poly_Rq_mul_tDiTRU(tmp, invgf, f, False);

  poly_Sq_mul_tDiTRU(invh, tmp, f); //need to write this for tditru (order)
  poly_Sq_tobytes(sk+2*tDiTRU_PACK_TRINARY_BYTES, invh);

  poly_Rq_mul_tDiTRU(tmp, invgf, g, False);
